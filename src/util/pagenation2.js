...
class Pagenation2 extends Component {
    ...
    render() {   
        return (
            <>
                <button onClick={this.leftPageClicked} disabled={this.state.startPage === 1}>&lt;</button>
                ...
                <button onClick={this.rightPageClicked} disabled={this.state.endPage === this.maxPage}>&gt;</button>            
            </>
        );
    }
}
...
class Numbering extends Component {
    ...
    render() {
        return (
            <button  onClick={() => this.props.clickListener(this.props.page)} style={this.props.page==this.props.currentPage ? {color:'red'} : {color:'black'}}>
                {this.props.page}
            </button>
        );
    }
